...
//     .catch((error)=>console.log(error));
    ...
    */
        ...
        const turant = Promise.resolve("Turant"); // This creates a promise that is already resolved with the value "Turant". The .then() method can be used to handle the resolved value immediately, without needing to wait for any asynchronous operation to complete.
        // console.log(turant);    // This will log a Promise object that is already resolved with the value "Turant". The promise is in the fulfilled state, and its value is available immediately.
        ...
        // any ka kaam hi ye hai ki agar any ek promise resolve ho jaye to uska result de dega, aur agar sare promises reject ho jaye to aggregate error dega, isliye is case me "Chai" aur "Code" dono resolve ho rahe hai, lekin "Chai" pehle resolve ho raha hai, isliye Promise.any() uska result return karega.
        // allPromise.then(console.log); // This will log "Chai" because the Promise.any() method returns a promise that fulfills as soon as any of the promises in the iterable fulfills. In this case, both "Chai" and "Code" are resolved promises, but "Chai" is the first one to fulfill, so it is the value that is logged. The rejected promise with the message "Error" is ignored since Promise.any() only cares about the first fulfilled promise. If all promises were to reject, then Promise.any() would reject with an AggregateError containing all the rejection reasons.
        
        const allPromise1 = Promise.all([
            Promise.resolve("Chai"),
            Promise.resolve("Code"),
            // Promise.reject("Error")
        ]); // This will create a promise that fulfills when all of the promises in the iterable have fulfilled, or rejects if any of the promises reject. In this case, since both "Chai" and "Code" are resolved promises, the resulting promise will fulfill with an array containing their values: ["Chai", "Code"]. If we were to include the rejected promise with the message "Error", then the resulting promise would reject with that error message instead.
        // allPromise1.then(console.log); // This will log ["Chai", "Code"] because the Promise.all() method returns a promise that fulfills when all of the promises in the iterable have fulfilled. In this case, both "Chai" and "Code" are resolved promises, so the resulting promise fulfills with an array containing their values. If any of the promises were to reject, then the resulting promise would reject with that error message instead.
        ...
        // allPromise2.then(console.log); // This will log an array of objects representing the outcome of each promise in the iterable. The first two promises are resolved, so they will have a status of "fulfilled" and their respective values. The third promise is rejected, so it will have a status of "rejected" and its reason for rejection. The output will look like this:
        /*
        [
        ...
        */
        ...
        // nice(); 
        // This will log a Promise object in the pending state immediately, because the hpromise is still waiting for the asynchronous operation to complete. After 3 seconds, when the promise is resolved with the value "Masterji", the result variable will be updated with the resolved value, but since we are logging it immediately after assigning it, it will still show as a pending promise. To see the resolved value, we would need to use .then() to handle the fulfillment of the promise and log the value when it becomes available.

        // This above we can write in a better way like this:

    const hpromise1 = new Promise((res,rej)=>{
        setTimeout(()=>{
            res("MASTERJI")
        },3000)
    })
    ...
    async function nice1(){ // 
        const result = await hpromise1
        console.log(result);
    }
    nice1();
    // This will log "MASTERJI" after 3 seconds. The async function nice1() allows us to use the await keyword to pause the execution of the function until the hpromise1 is resolved. Once the promise is resolved with the value "MASTERJI", it is assigned to the result variable, and then we log it to the console. This approach provides a more concise and readable way to handle asynchronous operations compared to using .then() chaining, especially when dealing with multiple asynchronous operations in a sequence.
    
    const hpromise2 = new Promise((res,rej)=>{
        setTimeout(()=>{
            rej("MASTERJI")
        },3000)
    })
    ...
    async function nice2(){ // 
        const result = await hpromise2
        console.log(result);
    }
    nice2(); // deko isme error aayega kyunki hpromise2 reject ho raha hai, aur jab hum await karte hai to agar promise reject hota hai to wo error throw karta hai, isliye hume is case me try-catch block ka use karke error ko handle karna chahiye, jisse hum graceful way me error ko manage kar sake.

    async function nice3(){
        try {
            const result = await hpromise2
            console.log(result);
        } catch (error) { // error kon pass karega? error hpromise2 se pass hoga jab wo reject hoga, aur wo error catch block me catch ho jayega, jisse hum usko handle kar sakte hai.
            console.log("Error caught:", error);
        } 
    }
    nice3(); // This will log "Error caught: MASTERJI" after 3 seconds. The try-catch block is used to handle the error that occurs when hpromise2 is rejected. When the promise is rejected with the reason "MASTERJI", it throws an error that is caught in the catch block, allowing us to log a custom error message along with the reason for rejection. This approach ensures that our code can handle errors gracefully without crashing, and it provides a clear way to manage exceptions in asynchronous code using async/await syntax.
